...
namespace MdTZ
{
    ...
    public sealed partial class MySqlHelper
    {
        ...
        public bool ExecuteBatchExt(List<String> SQLStringList)
        {
        ...
        public static int BulkInsert(string connectionString, DataTable table)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    bulk.Columns.AddRange(table.Columns.Cast<DataColumn>().Select(colum => colum.ColumnName).ToList().ToArray());
        ...
        //执行存储过程 
        /// <summary> 
        ///执行存储过程 PROCEDURE guopiao.error_test_proc does not exist"}
        /// </summary> 
        public int ExecuteProcNoOut(string procText)
        {
            using (MySqlConnection conn = new MySqlConnection(ConnectionString))
            {
                conn.Open();
                try
                {
                    MySqlCommand myCommand = new MySqlCommand(procText, conn);
        ...
        public  DataSet  ExecuteProc(string procName, MySqlParameter[] cmdParms)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(ConnectionString))
            {
                ...
                try
                {
                    ...
                    mysqldata.SelectCommand.CommandText = procName;
                    mysqldata.SelectCommand.CommandType = CommandType.StoredProcedure;

                    foreach(MySqlParameter param in cmdParms) {
                        mysqldata.SelectCommand.Parameters.Add(param);
                    }

                    mysqldata.Fill(dataset, procName);
